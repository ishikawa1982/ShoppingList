...
- 🔔 共有相手の「追加」「購入済み」をブラウザ通知（アプリ起動中。設定でオン）
...
設定の「通知」をオンにすると、共有相手の「追加」「購入済み」操作を**ブラウザ通知**で受け取れます
（自分の操作は通知しません）。

現状は **アプリが起動している間（フォアグラウンド／バックグラウンドのタブ）** に動作します。
これは追加費用・サーバー不要です。

> **アプリを完全に閉じている間にも届く本物のプッシュ**には、通知を「送信する係（サーバー）」が
> 必要です。Firebase の課金（Blaze）は不要で、**無料のサーバーレス（Cloudflare Workers / Deno Deploy 等、
> カード登録なしの無料枠）** で実装できます。仕組みは「変更した端末が、無料リレーに依頼 →
> リレーが他メンバーへ Web Push 送信」。この段階は次のステップとして用意予定です。
